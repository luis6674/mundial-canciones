...
<header class="site-header">
  <div class="header-inner">
    ...
    <nav class="main-nav">
      <?php if ($voting_open): ?>
        <span class="status-badge open">&#9679; Voting Open</span>
      <?php elseif ($voting_closed): ?>
        <span class="status-badge closed">&#9632; Voting Closed</span>
      <?php else: ?>
      ...
      <?php if ($logged_in): ?>
        <?php if ($voting_open): ?>
          <a href="vote.php" class="nav-link">My Votes</a>
        <?php endif; ?>
        <div class="user-chip">
          <?php if (!empty($user['avatar_url'])): ?>
            <img src="<?= htmlspecialchars($user['avatar_url']) ?>" alt="" class="avatar">
          <?php endif; ?>
          <span><?= htmlspecialchars($user['display_name'] ?? 'User') ?></span>
...
<footer class="site-footer">
  <div class="footer-inner">
    <p>&copy; <?= date('Y') ?> <?= htmlspecialchars(APP_NAME) ?> &mdash; Powered by music &amp; passion</p>
  </div>
</footer>
</body>
</html>
